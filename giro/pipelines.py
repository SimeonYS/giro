...
class GiroPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('giro.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `giro`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM giro WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO giro (date, title, link, content)"
